...
void writeIn(RegionInfo* pr, char* buf)
{
	...
	sscanf(buf, "%2s,%2s", pr->isoCountry, pr->regionCode); //Ôóíêöèÿ sscanf() èäåíòè÷íà ôóíêöèè scanf(), íî äàííûå ñ÷èòûâàþòñÿ èç ìàññèâà, óêàçàííîãî àðãóìåíòîì buf, à íå èç ôàéëà stdin.
	while (*buf++ != '"');
	while ((*pc++ = *buf++) != '"');
	*(--pc) = '\0'; //çàîäíî " óäàëèòñÿ
}
...
RegionInfo* fillList(RegionInfo* begin)
{
	...
	{
		if (feof(fp) || ferror(fp)) //×òåíèå ôàéëà çàêîí÷åíî èëè Îøèáêà ÷òåíèÿ èç ôàéëà
		{
			printf("er or EOF");
			break;
		}
...
void CountryISO_SP(RegionInfo* begin, char* countryISO)
{
	...
	while (begin != NULL)
	{
		if (!strcmp(begin->isoCountry, countryISO)) 				//0 – åñëè ñðàâíèâàåìåå ñòðîêè èäåíòè÷íû.														
		{
			flag = 0;
			printf("%s\,%s,\"%s\"\n", begin->isoCountry, begin->regionCode, begin->regionName);
		}
